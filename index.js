...
const nameMatch =
    message.content.match(/\*{0,2}name\*{0,2}\s*:\s*(.+)/i);

const steamMatch =
    message.content.match(/\*{0,2}steam\s*link\*{0,2}\s*:\s*(.+)/i);
        ...
        if (!interviewChannel) {
            console.log("INTERVIEW CHANNEL NOT FOUND");
            return;
        }
        ...
        console.log("INTERVIEW EMBED SENT");
