...
client.on(Events.InteractionCreate, async (interaction) => {
    ...
    if (interaction.customId.startsWith('rolemodal_')) {
        ...
        const acronym = interaction.fields.getTextInputValue('acronym');
        ...
        try {
            await member.setNickname(`[${acronym}] ${member.displayName}`);
        } catch (err) {
            console.log(err);
        }

        await interaction.reply({
            content: `✅ Approved with acronym: **${acronym}**`,
            ephemeral: true
        });
    }
}
...
if (interaction.customId.startsWith('finalapprove_')) {
    ...
    const modal = new ModalBuilder()
        .setCustomId(`rolemodal_${requesterId}_${roleId}`)
