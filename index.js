...
function createLogEmbed({ title, color, user, fields = [], extra = '' }) {
    return new EmbedBuilder()
        .setColor(color)
        .setTitle(`📡 ${title}`)
        .setThumbnail(user?.displayAvatarURL?.({ size: 1024 }) || null)
        .setAuthor({
            name: `${user?.tag || 'System'}`,
            iconURL: user?.displayAvatarURL?.({ size: 64 }) || null
        })
        .addFields(fields)
        .setFooter({ text: `User ID: ${user?.id || 'N/A'} ${extra}` })
        .setTimestamp();
}
...
client.on(Events.GuildMemberUpdate, (oldMember, newMember) => {
    ...
    if (added.size) {
        return sendLog(newMember.guild,
            createLogEmbed({
                title: "ROLE ADDED",
                color: "Green",
                user: newMember.user,
